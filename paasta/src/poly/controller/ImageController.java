...
/////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	/*
	 * 
	 * @RequestMapping(value = "/getImage", method = RequestMethod.GET) public void
	 * getImage(HttpServletRequest request, HttpSession session, HttpServletResponse
	 * response,
	 * 
	 * @RequestParam(value = "user_id") String user_id) throws Exception {
	 * 
	 * log.info("user_id : " + user_id);
	 * 
	 * // 가장 최근에 등록한 프로필 사진 정보가져오기 log.info("getImgList start! "); Map<String,
	 * String> pMap = imgService.getImgList(user_id); log.info("getImgList end! ");
	 * if (pMap == null) { pMap = new HashMap<String, String>(); } String realFile =
	 * nvl(pMap.get("SAVE_FILE_PATH") + "/"); // 파일이 저장된 경로 : /studyimg/0000/00/00/
	 * String fileNm = nvl(pMap.get("SAVE_FILE_NAME")); // 파일명 : 000000.jpg
	 * 000000.png String ext = nvl(pMap.get("EXT")); // 파일 확장자
	 * log.info("realFile : " + realFile); log.info("fileNm : " + fileNm);
	 * log.info("ext : " + ext);
	 * 
	 * BufferedOutputStream out = null; InputStream in = null;
	 * 
	 * try {
	 * 
	 * if (!ext.equals("")) { response.setContentType("image/" + ext);
	 * response.setHeader("Content-Disposition", "inline;filename=" + fileNm); File
	 * file = new File(realFile + fileNm); log.info("file : " + file);
	 * 
	 * in = new FileInputStream(file); out = new
	 * BufferedOutputStream(response.getOutputStream()); int len; byte[] buf = new
	 * byte[1024]; while ((len = in.read(buf)) > 0) { out.write(buf, 0, len); } }
	 * else { log.info("basicFile start");
	 * 
	 * String basicFile = "/imgg/basicimg.png";
	 * 
	 * File file1 = new File(basicFile); log.info("basicFile : " + basicFile);
	 * log.info("file1 : " + file1);
	 * 
	 * in = new FileInputStream(file1); out = new
	 * BufferedOutputStream(response.getOutputStream()); int len; byte[] buf = new
	 * byte[1024]; while ((len = in.read(buf)) > 0) { out.write(buf, 0, len); }
	 * 
	 * } } catch (Exception e) { log.info(e.getStackTrace()); } finally { if (out !=
	 * null) { out.flush(); } if (out != null) { out.close(); } if (in != null) {
	 * in.close(); } } }
	 * 
	 * // 스터디 이미지 불러오기 ( InputStream으로 파일 불러옴 )
	 * 
	 * @RequestMapping(value = "/getStudyImage", method = RequestMethod.GET) public
	 * void getStudyImage(HttpServletRequest request, HttpSession session,
	 ...
	 * log.info("study_name : " + study_name);
	 * 
	 * // 가장 최근에 등록한 프로필 사진 정보가져오기 log.info("getStudyImgList start! "); Map<String,
	 * String> pMap = imgService.getStudyImgList(study_name);
	 * log.info("getStudyImgList end! ");
	 * 
	 * if(pMap == null) { pMap = new HashMap<String, String>(); } String realFile =
	 * nvl(pMap.get("SAVE_FILE_PATH") + "/"); // 파일이 저장된 경로 C:\\upload\\ String
	 * fileNm = nvl(pMap.get("SAVE_FILE_NAME")); // 파일명 String ext =
	 * nvl(pMap.get("EXT")); // 파일 확장자 log.info("realFile : " + realFile);
	 * log.info("fileNm : " + fileNm); log.info("ext : " + ext);
	 * 
	 * BufferedOutputStream out = null; InputStream in = null;
	 * 
	 * 
	 * try { if (!ext.equals("")) { response.setContentType("image/" + ext);
	 * response.setHeader("Content-Disposition", "inline;filename=" + fileNm); File
	 * file = new File(realFile + fileNm); log.info("file : " + file);
	 * 
	 * in = new FileInputStream(file); out = new
	 * BufferedOutputStream(response.getOutputStream()); int len; byte[] buf = new
	 * byte[1024]; while ((len = in.read(buf)) > 0) { out.write(buf, 0, len); } }
	 * else { log.info("basicFile start"); String basicFile = "/imgg/basicimg.png";
	 * 
	 * File file1 = new File(basicFile); log.info("basicFile : " + basicFile);
	 * log.info("file1 : " + file1);
	 * 
